...
public:
    ...
    bool operator<=(const TreeNode& other) const {
        if (loc.x <= other.loc.x && loc.y <= other.loc.y) {
            return true;
        }
        return false;
    }
    ...
    DTYPE WireToSource() const {
        DTYPE total_len = 0;
        cout << id << ", ";
        if (parent) {
            total_len += WireToParent();
            cout << parent->id << ", ";
            auto parent_parent = parent->parent;
            while (parent_parent) {
                total_len += parent_parent->WireToParentChecked();
                cout << parent_parent->id << ", ";
...
public:
    ...
    // Traverse
    // void PreOrder(std::shared_ptr<salt::TreeNode> start_node, const function<void(const shared_ptr<TreeNode>&)>& visit) { if (start_node) TreeNode::PreOrder(start_node, visit); }
