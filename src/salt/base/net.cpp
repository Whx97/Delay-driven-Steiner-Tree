...
bool Net::Read(istream& is) {
    // header
    string buf, option, option_slack;
    int numPin = 0;
    ...
    iss >> id >> name >> numPin >> option >> option_slack;
    assert(numPin > 0);
    withCap = (option == "-cap");
    withSlack = (option_slack == "-slack");
    ...
    for (auto& pin : pins) {
        is >> i >> x >> y;
        if (withCap) is >> c;
        if (withSlack) is >> slack;
        pin = make_shared<Pin>(x, y, i, c, slack);
    }
